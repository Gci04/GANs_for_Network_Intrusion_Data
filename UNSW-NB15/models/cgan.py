...
class CGAN:
    ...
    def train(self):
        ...
        for epoch in tqdm.tqdm(range(self.tot_epochs), desc="cGAN training "):
            ...
            # Train Generator (generator in combined model is trainable while discriminator is frozen)
            for j in range(self.G_epochs):
                # Condition on labels
                sampled_labels = np.random.choice(
                    self.to_generate, (self.batch_size, 1), replace=True
                )
            ...
            # Print metrics print ("Epoch : {:d} [D loss: {:.4f}, acc.: {:.4f}] [G loss: {:.4f}]".format(epoch,
