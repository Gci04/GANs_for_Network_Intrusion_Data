...
def compare_classifiers(x_old, y_old, x_test, y_test, data_generator, label_mapping, models, cv=3):
    """Compares the performance of models using recall, precision & f-score. Dumps the results in .txt file"""
    ...
    performance_results = defaultdict(defaultdict(dict).copy)
    val, count = np.unique(y_old, return_counts=True)
    current_gt = dict(zip(val, count))
    up_sampling_strategy = {
        label_mapping.get(j): i + current_gt.get(label_mapping.get(j))
        for j, i in a.items()
        if i > 0
    }
    # up_sampling_strategy = {}
    ...
    if isinstance(data_generator, str):
        ...
        up_sampling_strategy = {
            label_mapping["probe"]: 14656,
            label_mapping["r2l"]: 13995,
            label_mapping["u2r"]: 10052,
        }
        if data_generator == "ADASYN":
            ...
            sm = SVMSMOTE(
                sampling_strategy=up_sampling_strategy,
                svm_estimator=SVC(C=10, cache_size=1500, class_weight="balanced"),
            )
    ...
    rus = RandomUnderSampler(
        sampling_strategy={
            label_mapping["Generic"]: 20000,
            label_mapping["Exploits"]: 20000,
        },
        random_state=42,
    )
    ...
    for i in range(cv):
        ...
        if not isinstance(data_generator, str):
            labels = np.random.choice(
                list(label_mapping.values()), (temp.sum(), 1), p=p, replace=True
            )
        ...
        svm_clf = svm(new_trainx, new_y, x_test, y_test, label_mapping, True)

        for estimator in [randf, deci, nn, svm_clf]:
            name = estimator.__class__.__name__
            pred = estimator.predict(x_test)
            precision, recall, fscore, _ = precision_recall_fscore_support(
                y_test, pred, labels=list(label_mapping.values())
            )
            performance_results[name][i]["precision"] = precision.tolist()
            performance_results[name][i]["recall"] = recall.tolist()
            performance_results[name][i]["fscore"] = fscore.tolist()
            performance_results[name][i]["weighted_f1"] = [
                f1_score(
                    y_test,
                    pred,
    ...
    t = int(time())
    for estimator in performance_results.keys():
        tempdf = pd.DataFrame.from_dict(performance_results[estimator][0])
        tempdf.index = list(label_mapping.values())

        for i in range(1, cv):
            to_append = pd.DataFrame.from_dict(performance_results[estimator][i])
        ...
        tempdf = tempdf.groupby("class").agg(
            {
                "precision": ["mean", "std"],
                "recall": ["mean", "std"],
                "fscore": ["mean", "std"],
...
def subsample(x, y, size=25000):
    ...
    for array, count in zip(class_wise_arrays, new_counts):
        class_wise_random_indices.append(
            np.random.choice(array.shape[0], int(count), replace=False)
        )

    data_small = np.vstack(
        tuple(
            [array[index, :] for array, index in zip(class_wise_arrays, class_wise_random_indices)]
        )
    )
...
def PlotPCA(n_components, X, y, label_mapping, save_dir="./unsw_pcaplots"):
    """Visualise clusters of data x, clusters labels y. PCA for reducing x to 2D"""
...
def plot_training_summary(filePath="", savePath=".s/imgs"):
    """
    Plot and Save GAN training metrics i.e discriminator, generator loss & accuracy, KL-Divergence
    :param filePath : sting, path to file containing GAN training logs file
