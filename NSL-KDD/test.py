...
def main():
    ...
    clf.DISPLAY_PERFORMANCE = False

    x = x_train[data_cols].values[
        att_ind
    ]  # x_train.query(f'label == {label_mapping["normal"]}').values
    y = y_train[att_ind]
    ...
    # for smoteMethod in ["ADASYN","SMOTEENN","BorderlineSMOTE", "SMOTE"]: clf.compare(x,y, x_test[data_cols].values[
    # for_test], y_test[for_test], smoteMethod, label_mapping, ml_classifiers ,cv=5) #SMOTE
