...
class Vannila_GAN():
    ...
    def __define_models(self):
        ...
        self.combined_model.compile(loss='binary_crossentropy',optimizer=self.optimizer, metrics=['accuracy'])
    ...
    def train(self,plot_dist = False):
        print("Vannila GAN Training : [Started]")
        ...
        for i in range(self.tot_epochs):
            ...
            for j in range(self.D_epochs):
                ...
                d_loss_real = self.discriminator.train_on_batch(x, real_labels)
            ...
            if i % 10 == 0:
                ...
                self.kl_history.append(np.sum(np.where(norm_p != 0, norm_p * np.log(norm_p/norm_q),0)))

                print(f"Epoch : {i}/{self.tot_epochs}")
                # print("Epoch : {:d} [D loss : {:.4f}, acc_fake : {:.4f}, acc_real : {:.4f}] [G loss : {:.4f}], KL : {:.4f}".format(i, d_loss,self.acc_history[0][0] ,self.acc_history[0][1], g_loss[0],kl_history[-1]))

        self.trained = True
        print("Vannila GAN Training : [Ended]")
    ...
    def save_model_componets(self,dir='vannilaGanLogs'):
        ...
        H["discriminator_loss"] = self.discriminator_loss
