...
def __display_perfomance(ytrue, ypred, labels_mapping):
    ...
    print(
        classification_report(
            ...
            target_names=list(labels_mapping.keys()),
        )
    )
...
def compare(
    ...
    for i in range(cv):
        ...
        for estimator in [randf, deci, nn, supvm]:
            ...
            precision, recall, fscore, _ = precision_recall_fscore_support(
                y_test, pred, labels=[0, 2, 3, 4]
            )
            performance_results[classifier_name][i]["precision"] = precision.tolist()
            performance_results[classifier_name][i]["recall"] = recall.tolist()
            performance_results[classifier_name][i]["fscore"] = fscore.tolist()
            performance_results[classifier_name][i]["weighted_f1"] = [
                f1_score(y_test, pred, labels=[0, 2, 3, 4], average="weighted")
            ] * len(label_mapping)
    ...
    for estimator in performance_results.keys():
        ...
        weighted_f1 = [
            f1_score(y_test, pred, labels=list(label_mapping.values()), average="weighted")
        ] * len(label_mapping)
        before_balance = pd.DataFrame(
            data=np.stack([precision, recall, fscore, weighted_f1]).T,
