...
from tensorflow.keras.layers import Activation, Dense, Dropout, Input, concatenate
...
class CGAN:
    ...
    def build_discriminator(self, x):
        """Create the discriminator model D(G(z,l)) : z -> random noise , l -> label (condition)"""
    ...
    def __define_models(self):
        ...
        self.generator = Model(
            inputs=[generator_input, labels_tensor],
        ...
        self.discriminator = Model(
            inputs=[discriminator_model_input],
    ...
    def __get_batch_idx(self):
        ...
        batch_ix = np.random.choice(
            len(self.X_train), size=self.batch_size, replace=True, p=self.sample_prob
        )
        ...
        for epoch in tqdm.tqdm(range(self.tot_epochs), desc="GAN Train loop"):
            ...
            for i in range(self.D_epochs):
                ...
                d_loss_real = self.discriminator.train_on_batch(
                    np.concatenate((x, labels), axis=1), real_labels
                )
            ...
            for j in range(self.G_epochs):
                ...
                sampled_labels = np.random.choice(
                    np.unique(self.y_train), (self.batch_size, 1), replace=True
                )
